...
<head>
    ...
    <style>
        ...
        /*
           FIX: Use absolute positioning for the sidebar to force 100% height.
           This stops dompdf from miscalculating table cell loops.
        */
        .sidebar {
            position: absolute;
            left: 0px;
        ...
        /*
           FIX: Use a normal container with a left margin to perfectly
           offset the absolute sidebar.
        */
        .content {
            margin-left: 34%;
            /* Leaves breathing room from the sidebar */
            padding: 40px 30px;
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-section">
            ...
            <div class="sidebar-item"><strong>DOB:</strong>
                {{ $user->profile->dob ? \Carbon\Carbon::parse($user->profile->dob)->format('d M, Y') : 'N/A' }}</div>
            <div class="sidebar-item"><strong>Gender:</strong> {{ $user->profile->gender ?? 'N/A' }}</div>
            <div class="sidebar-item"><strong>Nationality:</strong> {{ $user->profile->nationality ?? 'N/A' }}</div>
    ...
    <div class="content">
        ...
        @forelse($user->experiences as $exp)
            <div class="item-row">
                ...
                <div class="item-sub">{{ $exp->company_name }} | {{ $exp->employment_type }} - {{ $exp->location }}
                </div>
            ...
            <div class="item-row">
                <table class="item-header-table">
                    <tr>
                        ...
                        <td class="item-meta">{{ $ach->date ? \Carbon\Carbon::parse($ach->date)->format('M Y') : '' }}
                        </td>
                    </tr>
                </table>
